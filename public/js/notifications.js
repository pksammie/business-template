...
function openContextMenu(x, y, id, showSelect = true){

    contextTargetId = id;

    contextSelectBtn.style.display = showSelect ? "" : "none";
...
card.addEventListener("touchstart", (e) => {
    ...
    longPressTimer = setTimeout(() => {
        ...
        // On mobile, long-press already IS the selection gesture -- it
        // shouldn't also drop the whole list into multi-select mode.
        // Just offer a delete action for this one notification.
        if (selectionMode) {
            toggleSelected(notification.id, card);
        } else {
            openContextMenu(touchX, touchY, notification.id, false);
        }
