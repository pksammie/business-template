...
async function renderTabularCart() {
  console.log("RENDER CART", Date.now());
...
=================================== */
    ...
    */
      ...
      if (needsUpdate) {
        await setDoc(
          doc(db, "users", user.uid, "cart", item.firestoreId),
    ...
    if (cartUpdated) {
      showToast("Cart refreshed with latest product information.");
...
`;
      ...
      if (editMode) {
        card.addEventListener("click", () => {
          document.querySelectorAll(".cart-product-card").forEach((el) => {
...
window.actionUpdateCartRedirect = async function () {
  ...
  if (productSnap.data().isSuspended) {
    showToast("This product is unavailable.");

    return;
  }

  const selectedProduct = firestoreCart[selectedIndex];
...
if (restoreBtn) {
  restoreBtn.addEventListener("click", async () => {
    ...
    backups = backups.filter((backup) => {
      ...
      return now - backup.createdAt <= hours * 60 * 60 * 1000;
    });

    if (backups.length === 0) {
      showToast("No cart backups found.");
