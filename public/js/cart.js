...
async function renderTabularCart() {
  ...
  try {
    ...
    /* Product freshness (price/title/image sync + availability) is now
       handled once, in parallel, inside the cart snapshot listener --
       doing it here too (sequentially, per item) was the main reason
       the cart hung for 10-20+ seconds with a large cart: every single
       render re-issued one Firestore read per item, one at a time. */
      ...
      card.innerHTML = `
        ...
        <div class="cart-card-content">
          ...
          <div class="qty-control">
            <button onclick="event.stopPropagation(); decreaseCartQty('${item.firestoreId}')" class="luxury-qty-btn">−</button>
...
window.removeLineItem = function (index) {
  showConfirmModal("Remove this item from your bag?", async () => {
    ...
    // The cart listener will re-render once Firestore confirms the delete;
    // rendering here too would use the local array before it's caught up,
    // which was causing the item to briefly reappear.
  });
};
...
window.clearCart = function () {
  ...
  showConfirmModal("Clear your entire shopping bag?", async () => {
    ...
    firestoreCart.forEach(item => {
      batch.delete(doc(db, "users", auth.currentUser.uid, "cart", item.firestoreId));
    });
    await batch.commit();

    selectedCheckoutItems = [];
    showToast("Cart cleared.");
    // The cart listener will pick up the batch delete and re-render itself;
    // no need to render here off a local array that hasn't caught up yet.
  });
};
...
onAuthStateChanged(auth, user => {
    ...
    onSnapshot(
        ...
        async(snapshot)=>{
            ...
            const myGeneration = ++cartRenderGeneration;
            ...
            /* Fetch every item's product doc in parallel instead of one
               at a time -- with a large cart, sequential awaits here were
               the main reason clearing/removing items felt like it hung. */
            const productSnaps = await Promise.all(
                rawItems.map(item =>
                    item.productId
                        ? getDoc(doc(db, "products", item.productId)).catch(() => null)
                        : Promise.resolve(null)
                )
            );

            /* If a newer snapshot has already started (and possibly
               finished) while we were awaiting, drop this stale result
               instead of letting it render over the newer state. */
            if (myGeneration !== cartRenderGeneration) return;
            ...
            rawItems.forEach((item, i) => {
                ...
                const cartDocRef = doc(db, "users", user.uid, "cart", item.firestoreId);

                if (!pSnap || !pSnap.exists() || pSnap.data().isSuspended) {
            ...
            if (hasBatchedWrites) {
                try {
                    ...
                    console.error("Failed to sync cart item info", err);
                }
                if (myGeneration !== cartRenderGeneration) return;
            }

            firestoreCart = nextCart;
