...
const reviewPhotoGallery = document.getElementById("review-photo-gallery");
const addReviewPhotoBtn = document.getElementById("add-review-photo-btn");

function renderReviewPhotoGallery() {
  reviewPhotoGallery.innerHTML = uploadedReviewImages
    .map((url, i) => `
      <div class="review-photo-thumb">
        <img src="${url}">
        <button type="button" class="review-photo-remove" data-index="${i}">
  ...
  reviewPhotoGallery.querySelectorAll(".review-photo-remove").forEach(btn => {
    btn.addEventListener("click", () => {
      uploadedReviewImages.splice(Number(btn.dataset.index), 1);
      renderReviewPhotoGallery();
    });
  });

  addReviewPhotoBtn.disabled = uploadedReviewImages.length >= 3;
  addReviewPhotoBtn.innerHTML = uploadedReviewImages.length >= 3
    ? `<i class="fa-solid fa-check"></i> Maximum 3 photos`
    : `<i class="fa-solid fa-camera"></i> Add Photo`;
}

addReviewPhotoBtn?.addEventListener("click", () => {
  if (uploadedReviewImages.length >= 3) return;
  ...
  cloudinary.openUploadWidget(
    ...
    (error, result) => {
      ...
      if (result.event === "success") {
        uploadedReviewImages.push(result.info.secure_url);
        renderReviewPhotoGallery();
      }
    },
  );
...
await addDoc(
    ...
    /* update product rating stats — only for auto-approved reviews.
       A pending (1 or 2-star) review isn't counted yet; admin approving it
       later is what adds it to the aggregate (see admin-reviews.js). */
    if (rating > 2) {
...
reviewForm.addEventListener("submit", async e => {
  ...
  if (rating <= 2) { oneStarModal.style.display = "flex"; return; }
